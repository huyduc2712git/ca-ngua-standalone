...
test('A single host.mjs in an otherwise empty directory serves every embedded asset and creates rooms',async t=>{
  ...
  const frontend='https://embedded-game.vercel.app';
  const child=spawn(process.execPath,['host.mjs'],{cwd:dir,env:{...process.env,HOST:'127.0.0.1',PORT:'0',ROOMS_FILE:'',TRUST_PROXY:'',ALLOWED_ORIGINS:frontend},stdio:['ignore','pipe','pipe']});
  ...
  assert.deepEqual(await health.json(),{ok:true,version:'2.1.1',persistence:false});
  ...
  const preflight=await fetch(origin+'/api/rooms',{method:'OPTIONS',headers:{Origin:frontend,'Access-Control-Request-Method':'POST','Access-Control-Request-Headers':'content-type,x-player-token'}});
  assert.equal(preflight.status,204);assert.equal(preflight.headers.get('access-control-allow-origin'),frontend);
  const created=await fetch(origin+'/api/rooms',{method:'POST',headers:{'Content-Type':'application/json',Origin:frontend},body:JSON.stringify({name:'Host Render',capacity:2})});
  assert.equal(created.headers.get('access-control-allow-origin'),frontend);
