...
export function createRoomStore(file,now=()=>Date.now()){
  ...
  function read(){
    ...
    for(const r of input.rooms){
      ...
      if(r.game){const g=r.game;if(!['vn-one-die-1.0','vn-two-dice-1.0'].includes(g.rules)||!Array.isArray(g.players)||g.players.length<2||g.players.length>4||!Number.isInteger(g.current)||g.current<0||g.current>=g.players.length||!['playing','finished'].includes(g.status)||!['roll','move','finished'].includes(g.phase))throw new Error('Ván đã lưu không tương thích.');for(const p of g.players)if(!Array.isArray(p.pieces)||p.pieces.length!==4||p.pieces.some(v=>!Number.isInteger(v)||v< -1||v>61))throw new Error('Vị trí ngựa đã lưu không hợp lệ.');}
    }
